      ...
      <div className="absolute bottom-0 h-[80%] flex flex-col w-full bg-white shadow-top rounded-tl-[20px] rounded-tr-[20px] py-4 px-8 animate-slideUp">
        ...
        <div className="rounded-[10px] bg-[#FFF9D7] w-full p-5 px-12 flex flex-row">
          ...
          <div className="flex flex-col justify-center items-center flex-grow">
            <div className="mb-1">{parentInfo ? `${parentInfo.child.kindergartenClass.kindergarten.kindergatendName} ${parentInfo.child.kindergartenClass.kindergartenClassName}` : ""}</div>
            <div className="font-semibold text-[18px]">{parentInfo ? `${parentInfo.child.name}` : ""}</div>
          </div>
        </div>
      </div>
    </div>
